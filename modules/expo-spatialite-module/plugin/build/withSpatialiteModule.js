...
const withSpatialiteModule = (config) => {
    ...
    repositories {
        ...
        if (!config.modResults.contents.includes('google()')) {
            config.modResults.contents = config.modResults.contents.replace(/(allprojects\s*{[^}]*repositories\s*{)/, `$1
        google()`);
        }
        return config;
    });
    ...
    config = withAppBuildGradle(config, (config) => {
        ...
        if (!config.modResults.contents.includes('com.github.mvits:Geo-Spatialite-Android')) {
            config.modResults.contents = config.modResults.contents.replace(/(dependencies\s*{)/, `$1
    ...
    implementation 'androidx.appcompat:appcompat:1.6.1'`);
        ...
        if (!config.modResults.contents.includes('packagingOptions')) {
            config.modResults.contents = config.modResults.contents.replace(/(android\s*{)/, `$1
    packagingOptions {
